...
include "../../include/circomlib/circuits/switcher.circom";
...
template LayoutEligibility(MAP_WIDTH, MAP_HEIGHT, MAX_HOUSE, MAX_HOUSE_WIDTH, MAX_HOUSE_HEIGHT, MAX_PATH_SIZE) {
  ...
  component ell_1 = LessThan(5);
  ell_1.in[0] <== out2calc;
  ell_1.in[1] <== compare2;
  (out2, t2) <== Switcher()(ell_1.out, out2calc, compare2);
  ...
  component ell_2 = LessThan(5);
  ell_2.in[0] <== out3calc;
  ell_2.in[1] <== compare3;
  (out3, t3) <== Switcher()(ell_2.out, out3calc, compare3);

  rate <== out3;

  // at least one seaport
  component gt_1 = GreaterThan(5);
  gt_1.in[0] <== type3num;
  gt_1.in[1] <== 0;
